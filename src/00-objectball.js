...
function playerNumbers(teamName) {
  ...
  if (teamName === gameObject.home.teamName) {
    return [
      playersListHome.AlanAnderson.Number,
      playersListHome.BrookLopez.Number,
      playersListHome.JasonTerry.Number,
      playersListHome.MasonPlumlee.Number,
      playersListHome.ReggieEvans.Number,
    ];
  } else if (teamName === gameObject.away.teamName) {
    return [
      playersListAway.BenGordon.Number,
      playersListAway.BismakBiyombo.Number,
      playersListAway.BrendanHaywood.Number,
      playersListAway.DeSagnaDiop.Number,
      playersListAway.JeffAdrien.Number,
    ];
  }
}
...
function bigShoeRebounds() {
  const shoeNumbersOfHome = Object.values(gameObject.home.players).map(
    function (item) {
      return item.Shoe;
    }
  ); // We got shoe numbers of all players in home team

  const biggestShoeOfHomePlayers = shoeNumbersOfHome.reduce(
    (preValue, curValue) => {
  ...
  const shoeNumbersOfAway = Object.values(gameObject.away.players).map(
    function (item) {
      return item.Shoe;
    }
  );

  const biggestShoeOfAwayPlayers = shoeNumbersOfAway.reduce(
    (preValue, curValue) => {
      if (preValue > curValue) {
        return preValue;
      } else {
  ...
  if (biggestShoeOfHomePlayers > biggestShoeOfAwayPlayers) {
    const shoeOwnersStats = Object.values(gameObject.home.players).find(
      function (item) {
    ...
    const shoeOwnersStats = Object.values(gameObject.away.players).find(
      function (item) {
...
function mostPointsScored() {
  const biggestScoreHome = Object.values(gameObject.home.players)
    .map(function (item) {
  ...
  const whoseScoreIsThisHome = Object.values(gameObject.home.players).find(
    (item) => {
  ...
  const mostScoredPlayerHome = getKeyByValue(
    gameObject.home.players,
    whoseScoreIsThisHome
  ); // By using the function, we found who is this person in home team

  const biggestScoreAway = Object.values(gameObject.away.players)
    .map(function (item) {
      return item.Points; // We returned an array of scores belong to each player
    })
    .reduce(function (value1, value2) {
  ...
  const whoseScoreIsThisAway = Object.values(gameObject.away.players).find(
    (item) => {
  ...
  const mostScoredPlayerAway = getKeyByValue(
    gameObject.away.players,
    whoseScoreIsThisAway
  );
...
function winningTeam() {
  const pointsOfHome = Object.values(gameObject.home.players)
    .map(function (player) {
  ...
  const pointsOfAway = Object.values(gameObject.away.players)
    .map(function (player) {
      return player.Points; // We returned an array which includes each players score number
    })
    .reduce(function (preValue, curValue) {
...
function playerWithLongestName() {
  const longestNameHome = Object.keys(gameObject.home.players)
    .map(function (nameOfPlayer) {
  ...
  const longestNameAway = Object.keys(gameObject.away.players)
    .map(function (nameOfPlayer) {
...
function mostStealPlayer() {
  const mostStealHome = Object.values(gameObject.home.players)
    .map(function (item) {
  ...
  const whoIsThisSteallerHome = Object.values(gameObject.home.players).find(
    (item) => {
  ...
  const mostStealedPlayerHome = getKeyByValue(
    gameObject.home.players,
    whoIsThisSteallerHome
  ); // By using the function, we found who is this person in home team

  const mostStealAway = Object.values(gameObject.away.players)
    .map(function (item) {
  ...
  const whoIsThisSteallerAway = Object.values(gameObject.away.players).find(
    (item) => {
  ...
  const mostStealedPlayerAway = getKeyByValue(
    gameObject.away.players,
    whoIsThisSteallerAway
  );
